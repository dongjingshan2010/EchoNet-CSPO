...
# ─────────────────────────────────────────────────────────────────────────────
# VideosA4C 专用数据集（适配小儿超声，不依赖 EchoNoLabelDataset）
# ─────────────────────────────────────────────────────────────────────────────

class VideosA4CPediatricDataset:
    """轻量 Dataset，适配 VideosA4C CSV 格式差异：
      - FileName 已含 .avi，直接拼路径
      - 无 NumberOfFrames / EDV / ESV 列
      - Split 为数值 0-9；split_val='ALL' 使用全部数据
    暴露 .df 属性供 _select_videos() 使用。
    """
    ...
    def __init__(self, filelist: str, videos_dir: str, cfg, split_val: str = 'ALL'):
        ...
        if split_val != 'ALL':
            try:
                df = df[df['Split'] == int(split_val)].reset_index(drop=True)
            except ValueError:
                pass  # 无法解析则使用全量
        self.df = df[df['EF'].notna()].reset_index(drop=True)

        self.size      = int(cfg.data.frame_size)
        self.stride    = int(cfg.data.frame_stride)
        self.max_steps = int(cfg.data.max_steps)
        self.mean = np.array(cfg.data.normalize_mean, dtype=np.float32).reshape(1, 1, 1, 3)
        self.std  = np.array(cfg.data.normalize_std,  dtype=np.float32).reshape(1, 1, 1, 3)
    ...
    def _load(self, idx: int):
        ...
        vid_path = os.path.join(self.videos_dir, str(row['FileName']))  # 已含 .avi
        frames   = read_video(vid_path, self.size)
        T        = frames.shape[0]

        span    = self.max_steps * self.stride
        start   = 0 if T <= span else int(max(0, (T - span) // 2))
...
def run_real(args: argparse.Namespace) -> None:
    ...
    # ── Dataset (VideosA4C Pediatric) ─────────────────────────────────────────
    # 将 config 中的相对路径解析为绝对路径（相对于项目根目录 _ROOT）
    _filelist   = getattr(args, 'filelist',   None) or str(cfg.data.filelist)
    _videos_dir = getattr(args, 'videos_dir', None) or str(cfg.data.videos_dir)
    if not os.path.isabs(_filelist):
        _filelist = os.path.normpath(os.path.join(_ROOT, _filelist))
    if not os.path.isabs(_videos_dir):
        _videos_dir = os.path.normpath(os.path.join(_ROOT, _videos_dir))
    print(f"[real] filelist   = {_filelist}")
    print(f"[real] videos_dir = {_videos_dir}")

    dataset = VideosA4CPediatricDataset(
        filelist=_filelist, videos_dir=_videos_dir,
        cfg=cfg, split_val=args.split,
    )
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        "--filelist", default=None,
        metavar="CSV",
        help="覆盖 config 的 data.filelist（绝对路径或相对项目根的路径）",
    )
    p.add_argument(
        "--videos_dir", default=None,
        metavar="DIR",
        help="覆盖 config 的 data.videos_dir",
    )
    p.add_argument(
...
# ---------------------------------------------------------------------------
# ★ 直接在 PyCharm Run 时生效的变量，无需命令行参数。
#   修改下面几个值后按绿色 Run 即可。
# ---------------------------------------------------------------------------
DEMO      = False
CKPT      = r"../checkpoints/best.pt"
CONFIG    = r"../configs/default.yaml"
SPLIT     = "ALL"        # 'ALL'=全部数据；'0'~'9'=指定折
N_SAMPLES = 4            # 总视频数（sinus + arrhythmia 各半）
OUT_DIR   = r"../figures/ite_pediatric"
FILELIST   = r"E:\Heart\data\VideosA4C\FileList_with_frames.csv"
VIDEOS_DIR = r"E:\Heart\data\VideosA4C\VideosA4C"
# ---------------------------------------------------------------------------
...
if __name__ == "__main__":
    ...
    if _cli:
        ...
        args = _ap.Namespace(
            ...
            filelist=FILELIST, videos_dir=VIDEOS_DIR,
        )
        _demo    = DEMO
        _out_dir = OUT_DIR
