"""evaluate.py — CSPO 三动作独立评估脚本（含 ITE 角色归因报告）。
...
────
  ...
  python scripts/evaluate.py --ckpt checkpoints/best.pt --split VAL --max 200
  python scripts/evaluate.py --n_cspo 5   # FORCEDIVERGE 重复次数（平均 ITE）
"""
...
def print_metrics(metrics: dict, split: str):
    ...
    bar = '=' * 116
    print(f'\n{bar}')
    print(f'  EchoNet-CSPO-mamba3ActorPriod-R21D-CSPO  |  Split={split}  n={metrics["n"]}')
    print(bar)
    print(
...
def main():
    ...
    ap = argparse.ArgumentParser(
        description='EchoNet-CSPO-mamba3ActorPriod-R21D-derectEF-CSPO 独立评估脚本')
    ...
    ds = EchoNoLabelDataset(cfg, args.split)
    print(f'[info] {args.split} samples: {len(ds)}')
